...
export function SeoSidebar({
  ...
  return (
    <div className={`t-card rounded-2xl p-5 space-y-5 shadow-sm sticky top-20 border t-border`}>
      ...
      {/* Tab Switcher: Yoast Snippet vs Audit Checklist */}
      <div className="flex rounded-xl p-1 bg-stone-900/60 border t-border text-xs">
        <button
          type="button"
          onClick={() => setActiveTab("snippet")}
          className={`flex-1 py-1.5 px-3 rounded-lg font-semibold transition-all flex items-center justify-center gap-1.5 ${
            activeTab === "snippet" ? "bg-[#d97757] text-white shadow-sm" : `${tk.textMuted} hover:${tk.textPrimary}`
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-1.5 px-3 rounded-lg font-semibold transition-all flex items-center justify-center gap-1.5 ${
            activeTab === "checklist" ? "bg-[#d97757] text-white shadow-sm" : `${tk.textMuted} hover:${tk.textPrimary}`
          }`}
        >
          <ShieldCheck className="w-3.5 h-3.5" />
          <span>Audit Checklist ({defaultChecklist.filter((c) => c.passed).length}/{defaultChecklist.length})</span>
        </button>
      </div>
      ...
      {activeTab === "snippet" && (
        <div className="space-y-4 animate-in fade-in">
          ...
          <div className="rounded-xl border t-border bg-[#141210] p-3.5 space-y-1.5 font-sans">
            <div className="flex items-center gap-2 text-[10px] text-stone-400">
              <span className="w-4 h-4 rounded-full bg-stone-800 flex items-center justify-center text-[8px] text-[#d97757]">
                🌐
              </span>
              <span className="truncate">hariyuka.my.id › {activeSlug}</span>
            </div>
            <h4 className="text-xs font-semibold text-[#8ab4f8] hover:underline cursor-pointer leading-snug line-clamp-1">
              {activeSeoTitle}
            </h4>
            <p className="text-[11px] text-stone-400 leading-tight line-clamp-2">
              {activeMetaDesc}
            </p>
          </div>
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-[11px]">
                ...
                <button
                  ...
                  className={`flex items-center gap-1 text-[10px] ${tk.accentText} hover:underline font-medium cursor-pointer`}
                >
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-[11px]">
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-mono ${activeSeoTitle.length <= 60 ? "text-emerald-400" : "text-amber-400"}`}>
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-[11px]">
                ...
                <button
                  ...
                  className={`flex items-center gap-1 text-[10px] ${tk.accentText} hover:underline font-medium cursor-pointer`}
                >
                  {copiedKey === "slug" ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-[11px]">
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-mono ${activeMetaDesc.length >= 120 && activeMetaDesc.length <= 155 ? "text-emerald-400" : "text-amber-400"}`}>
            ...
            <button
              ...
              className={`w-full py-2.5 px-3 rounded-xl border border-[#d97757]/40 bg-[#d97757]/10 hover:bg-[#d97757] text-[#d97757] hover:text-white font-semibold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm`}
            >
